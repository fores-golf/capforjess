...
def main():
    ...
    all_api_events = []
    current_page = 1
    per_page = 50  # 50 is a very "safe" standard number that APIs won't block

    print("Fetching all events from Capital One (with Pagination)...")

    while True:
        api_url = f"https://entertainment-bff.capitalone.com/events?date_start={today}&tag_filter=C1_EXCLUSIVE+AND+NOT+COLLAPSED&page={current_page}&per_page={per_page}"
        ...
        if not page_items:
            # No more items found, we reached the end of the line!
            break

        all_api_events.extend(page_items)
        print(f"-> Retrieved {len(page_items)} events from page {current_page}...")

        # Capital One's API tells us how many total pages exist in the "number_of_pages" key
    ...
    if not all_api_events:
        print("No events gathered across any pages.")
        return

    print(f"Total events gathered: {len(all_api_events)}. Checking against database...")

    # 2. Track and process new events
